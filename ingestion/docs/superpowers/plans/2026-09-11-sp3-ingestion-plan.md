...
10. **Every verify command is `dotnet run --project <test csproj> -c Release -f net10.0 -p:TargetFrameworks=net10.0`, never `dotnet test`** — SP1 adjustment 36 and SP2 adjustment 12, unchanged. The two single-TFM test projects (`…Onnx.Tests`, `…DataIngestion.Tests`) take `-p:TargetFrameworks=net10.0` and **no** `-f`. **In a parallel phase every one of them also carries `-p:ArtifactsPath=D:\Local\Temp\qedge-sp3\w<wave>\t<task>`**; the wave's close re-runs the identical commands without it. **Exception — a device-TFM build carries `-f <tfm>` and NO `-p:TargetFrameworks`**, because `-p:` is a global property that would force every `net10.0`-only project in the closure to a TFM it does not have (`NETSDK1005`). (Every task.)
