...
21. **Tier 0 is Task 1.5 — the end of wave 1 — and not literally "the first task of the plan", because §16.0 contradicts itself.** §16.0 and §19 item 1 both require the five-target link-and-generate smoke to run before any SP4 code, and §16.0's own body makes "first" impossible: the smoke "loads the tier-2 fixture and generates one token", and the tier-2 fixture does not exist until **Task 1.3** builds it. Tier 0 therefore runs at the end of wave 1 — after the fixture, before one line of SP4 API — which is the property §16.0 actually cares about ("a TFM cut discovered after the API is written is a rewrite"). Four consequences, every one of them a task rather than a sentence:
    ...
    **Closing note (Task 1.6 Step 4, 2026-09-11).** All six legs are green: workflow run [`34637304766`](https://github.com/qavren-oss/qavren-edge/actions/runs/34637304766), dispatched against `feat/sp4-chat` HEAD `58dde19` by Task 1.6 Step 3's push-triggered dispatch. `tier0-windows` and `tier0-linux` print the smoke's own `key=value` block (`ichatClientAssignable=True`, `firstTokenId=33`, `firstTokenText=!`, `modelLoadMs=93.2` on Windows / `23.1` on Linux); the four device legs (`tier0-winui`, `tier0-android`, `tier0-ios`, `tier0-maccatalyst`) run the same smoke body as one xUnit device-runner fact and report `Total=1, Passed=1, Failed=0, Skipped=0` — the device-runner's visual-runner protocol surfaces pass/fail and VSTest totals to the job log, not the shared body's `key=value` prints. Full detail and the leg table now live in `chat/README.md`'s "What is proven and what is not" section.

    The immediately prior run, [`34635404329`](https://github.com/qavren-oss/qavren-edge/actions/runs/34635404329) (commit `77510db`), had five legs green and `tier0-android` red: the workflow's own job-level `DEVICE` environment variable — named for the smoke device-head csproj path — collided with MSBuild's `$(Device)` property, which the Android build's device-spec resolution (bundletool's `DeviceAnalyzer`, reached from `AdbTarget`) reads, so it tried to resolve the booted emulator against a device spec built from a csproj path string and failed with `error XABAS0000: [BT:1.17.0] Error: Unable to find the requested device.` The fix, landed in `58dde19`, renamed the workflow's env vars off `DEVICE` (`SMOKE` / `DEVICE_HEAD`); `tier0-android` has been green on every run since, including `34637304766` above. No fallback (adjustment 21(a)) was triggered — `tier0-maccatalyst` passed on both runs. Wave 2 is cleared to start per Step 5.
...
25. **§16.2's "byte counts asserted so an onnx or builder upgrade that moves them is loud" becomes a test, not a doc comment.** The draft put the byte counts in XML doc comments on the base64 consts and asserted only the 1 MB source cap plus a by-hand regeneration diff — and CI never runs the generator, so nothing anywhere would notice a moved byte count. `make_tiny_chat_model.py` therefore emits each file's length as a `const int` beside its base64 (`GenAiConfigJsonLength`, `ModelOnnxLength`, `ModelOnnxDataLength`, `TokenizerJsonLength`, `TokenizerConfigJsonLength`, `ChatTemplateJinjaLength`), and `Tier2/TinyChatModelFixtureTests` — which runs on every PR, every host and every device lane — asserts that each materialised file's on-disk length equals its `const`, that every base64 decode round-trips, and that the six lengths sum under the cap. That is the "loud" half, and it is the half that only exists if a *test* owns it. (Tasks 1.3, 6.1.)

    **Amended during Task 1.3 — the fixture is six files, not four, and both additions are load-bearing.** The draft named four because it assumed the builder would inline small initializers and that the chat template would ride inside `tokenizer_config.json`. Materialising only the four produces a directory that does not load, reproduced deliberately before the list was changed:

    - **`model.onnx.data`** — ORT GenAI 0.15.2's builder calls `ir.save(..., size_threshold_bytes=0)` (`builders/base.py:1197`), so **every** initializer goes external unconditionally and no flag turns it off: `model.onnx` is a 14,402-byte graph and the 394,496 bytes of weights live beside it. Without the file, `og.Model(dir)` throws `External data path does not exist`.
    - **`chat_template.jinja`** — transformers 5.17.0 no longer writes `chat_template` into `tokenizer_config.json` (the builder round-trips the tokenizer through transformers, leaving 263 bytes of backend metadata and no template), so `Tokenizer.ApplyChatTemplate` throws `Empty chat template` unless the template is its own file.
...
foreach ($n in 9,10,11,12,13) {
  if (-not (Get-ChildItem "$root\chat\docs\adr" -Filter ("{0:D4}-*.md" -f $n))) { throw ("ADR {0:D4} missing" -f $n) }
}
...
- **`TinyChatModelFixtureTests` — the byte counts, which §16.2 asks to be "loud" and adjustment 25 makes a test.** For each of the **six** fixture files — `GenAiConfigJsonLength`, `ModelOnnxLength`, `ModelOnnxDataLength`, `TokenizerJsonLength`, `TokenizerConfigJsonLength`, `ChatTemplateJinjaLength` — the materialised file's on-disk length equals the `const int …Length` the generator emitted beside its base64; every base64 decodes without throwing; and the six lengths sum below the 1 MB source cap. **Materialise all six**: a fixture written with only the four the draft named produces a directory that does not load (`model.onnx.data` carries every weight, `chat_template.jinja` carries the template — adjustment 25's amendment has the measurements). CI never runs `make_tiny_chat_model.py`, so this is the **only** thing in the repo that notices an onnx, torch, transformers or builder upgrade moving the serialised bytes — Task 1.3's regeneration diff fires only on a box where somebody re-ran the generator by hand. It runs on every PR, every host and every device lane.
