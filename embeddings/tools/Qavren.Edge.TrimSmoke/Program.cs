...
#pragma warning restore CA1031
    ...
    private static async Task<int> RunAsync(string root, string staging)
    {
        ...
        Log($"[1/10] staged  graph {graph.Length} B, vocab {vocabulary.Length} B under {staging}");
        ...
        await using (provider.ConfigureAwait(false))
        {
            ...
            Log($"[3/10] tokens  {count} ids, vocabulary {tokenizer.VocabularySize} entries: {Join(ids, count)}");
            ...
            Log($"[4/10] embed   [{Format(first.Span)}] and [{Format(second.Span)}]");
            ...
            using (collection)
            {
                ...
                Log("[5/10] upsert  2 rows into vec0");
                ...
                Log($"[6/10] search  {hits.Count} hits, nearest '{nearest}' at distance {hits[0].Score}");
            ...
            Log($"[7/10] ingest  {Describe(markdownRun)}");
            ...
            using (chunks)
            {
                ...
                Log($"[8/10] chunk   {chunkHits.Count} hits; nearest is {chunk.DocumentId} #{chunk.Ordinal} breadcrumb '{chunk.Breadcrumb}' chars [{chunk.CharStart}, {chunk.CharEnd}) {chunk.TokenCount} tokens, {chunk.BlockKind}, via '{chunk.ExtractorId}'");
            ...
            Log($"[9/10] pdf     {pdf.Length} B: {Describe(pdfRun)}");
            ...
            Log($"[9/10] docx    {docx.Length} B: {Describe(docxRun)}");
...
#else
            Log("[9/10] satellites not linked: core-only publish");
#endif
        }

        // 8. The RAG recipe, with no model, no natives and no weights. DelegateRetriever over an
    ...
    private static async Task<int> RunRagAsync()
    {
        ...
        Log($"[10/10] rag    {citations.Count} citations over {markers} markers, {answer.Length}-character answer");
        Log("OK: rag");
        return 0;
    }
